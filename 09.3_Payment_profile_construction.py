...
for decile_num in sorted(df['decile'].unique()):
    ...
    print(f"\n  DEBUG: n_late={n_late}, n_accounts={n_accounts}, prob_late={prob_late:.4f} ({prob_late*100:.1f}%)")
    ...
    if n_late > 0:
        ...
        for cd_level in all_cd_levels:
            ...
            prob_cd_late = cd_distribution_late.get(cd_level, 0)  # Renamed from prob_late
            cd_given_late[int(cd_level)] = float(prob_cd_late)
            print(f"  {cd_level:<4} {prob_all*100:>6.1f}%        {prob_cd_late*100:>6.1f}%")
    else:
    ...
    # ADD TO SUMMARY
    # ================================================================
    # DEBUG: Check prob_late value before adding to summary
    print(f"  DEBUG BEFORE SUMMARY: prob_late = {prob_late:.4f} ({prob_late*100:.1f}%)")
...
# Debug: Print what's actually being saved
print("\nDEBUG - Summary data being saved to CSV:")
print(summary_df[['decile', 'n_accounts', 'n_late', 'prob_late_pct']].to_string())
