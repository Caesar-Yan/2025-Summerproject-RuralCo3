'''
10.0.3_Monte_Carlo_with_rebundling.py

MONTE CARLO VERSION with RE-BUNDLING AT EACH ITERATION
Same as 10.0.3 but runs multiple simulations with rebundling

KEY FEATURES:
- Loads UNBUNDLED invoice data from 09.1
- Runs N_SIMULATIONS Monte Carlo simulations
- Re-bundles invoices at EACH iteration (introducing bundling variability)
- NO seasonal adjustments (unlike 10.5)
- Uses decile base rates directly from payment profile
...
Outputs:
- 10.0.3_MC_FY2025_comparison_summary_rebundled.csv
- 10.0.3_MC_all_rounds_rebundled.csv
- 10.0.3_MC_revenue_distribution_histogram_rebundled.png
- 10.0.3_MC_confidence_intervals_rebundled.png
- 10.0.3_MC_scenario_comparison_boxplot_rebundled.png
- 10.0.3_MC_monthly_revenue_uncertainty_rebundled.png
- 10.0.3_MC_detailed_simulations_rebundled.xlsx
...
print("\n" + "="*80)
print("MONTE CARLO SIMULATION WITH RE-BUNDLING (NO SEASONAL ADJUSTMENTS)")
print("Based on 10.0.3 methodology with 10.5's rebundling approach")
print("="*80)
print(f"Base Directory: {BASE_PATH}")
print(f"Annual Interest Rate: {ANNUAL_INTEREST_RATE*100:.2f}%")
print(f"Number of Simulations: {N_SIMULATIONS}")
print(f"Base Random Seed: {BASE_SEED}")
print(f"Bundling uncertainty: YES (rebundle each iteration)")
print(f"Seasonal adjustments: NO")
print("="*80)
...
# ================================================================
# STEP 1: Load UNBUNDLED Invoice Data from 09.1
# ================================================================
print("\n" + "="*80)
print("📂 [Step 1/4] LOADING UNBUNDLED INVOICE DATA FROM 09.1")
print("="*80)
...
# ================================================================
# STEP 2: Parse Dates and Filter to FY2025
# ================================================================
print("\n" + "="*80)
print("📅 [Step 2/4] PARSING DATES AND FILTERING TO FY2025")
print("="*80)
...
if len(fy2025_invoices) == 0:
    ...
    print(f"  Min: {all_invoices['invoice_period'].min()}")
    print(f"  Max: {all_invoices['invoice_period'].max()}")
    exit()

# ================================================================
# STEP 3: Load Decile Payment Profile
# ================================================================
print("\n" + "="*80)
print("📊 [Step 3/4] LOADING DECILE PAYMENT PROFILE")
print("="*80)
...
# Pre-compute decile base rates (NO seasonal adjustment)
decile_base_rates = {}
for i in range(n_deciles):
    ...
    if decile_key in decile_profile['deciles']:
        ...
        base_rate = 0.02  # Default fallback
...
for i in range(n_deciles):
    ...
    if decile_key in decile_profile['deciles']:
        ...
        if cd_given_late:
            ...
            cd_probs = cd_probs / cd_probs.sum()  # Normalize
            decile_cd_distributions[i] = (cd_levels, cd_probs)
        else:
            decile_cd_distributions[i] = ([3], [1.0])  # Default to cd=3
    else:
        decile_cd_distributions[i] = ([3], [1.0])

print(f"  ✓ Pre-computed decile base rates and cd distributions")
print(f"    Using decile base rates WITHOUT seasonal adjustments")

# ================================================================
# STEP 4: Define Bundling and Simulation Functions
# ================================================================
print("\n" + "="*80)
print("🔧 [Step 4/4] DEFINING BUNDLING AND SIMULATION FUNCTIONS")
print("="*80)
...
def bundle_invoices_to_statements(invoices_df, seed):
    ...
    Adapted from 9.7 to run within each simulation iteration
...
def simulate_both_scenarios_with_rebundling(invoices_df, seed):
    """
    VECTORIZED simulation with RE-BUNDLING at each iteration
    NO SEASONAL ADJUSTMENTS (unlike 10.5)
    
    Steps:
    1. Bundle invoices into statements (random groupings)
    2. Assign deciles based on statement totals
    3. Use decile base rates directly (NO seasonal adjustment)
    ...
    # ================================================================
    # STEP 2: Map decile base rates (NO seasonal adjustment)
    # ================================================================
    ...
    # Use base rate directly (NO seasonal adjustment)
    statements['late_rate'] = statements['decile_base_rate']
    
    # ================================================================
    # STEP 3: Determine late status (VECTORIZED)
    # ================================================================
    random_draws = np.random.random(len(statements))
    statements['is_late'] = random_draws < statements['late_rate']
    
    # ================================================================
    # STEP 4: Sample cd levels for late statements
    # ================================================================
    ...
    monthly_no['cumulative'] = monthly_no['cumulative_total']  # Add for consistency with monthly_with
...
for i in range(N_SIMULATIONS):
    ...
    # Save first round for detailed output
    if i == 0:
...
print(f"\n  ✓ Completed {N_SIMULATIONS} simulations in {elapsed:.2f} seconds")
print(f"    Average: {elapsed/N_SIMULATIONS:.3f}s per simulation")
...
if revenue_diff_mean > 0:
    ...
    pct_more = (abs(revenue_diff_mean) / summary_no['total_revenue_mean']) * 100
    print(f"\n  ✓ WITH DISCOUNT generates ${abs(revenue_diff_mean):,.2f} MORE revenue on average ({pct_more:.1f}%)")
...
summary_output = OUTPUT_DIR / '10.0.3_MC_FY2025_comparison_summary_rebundled.csv'
monte_carlo_summary_df.to_csv(summary_output, index=False)
print(f"  ✓ Saved: {summary_output.name}")

all_rounds_csv = OUTPUT_DIR / '10.0.3_MC_all_rounds_rebundled.csv'
summary_stats_df.to_csv(all_rounds_csv, index=False)
print(f"  ✓ Saved: {all_rounds_csv.name}")

output_excel = OUTPUT_DIR / '10.0.3_MC_detailed_simulations_rebundled.xlsx'
...
plt.tight_layout()
hist_output = OUTPUT_DIR / '10.0.3_MC_revenue_distribution_histogram_rebundled.png'
...
plt.tight_layout()
ci_output = OUTPUT_DIR / '10.0.3_MC_confidence_intervals_rebundled.png'
...
plt.tight_layout()
boxplot_output = OUTPUT_DIR / '10.0.3_MC_scenario_comparison_boxplot_rebundled.png'
...
# Add revenue labels along the lines
...
ax.set_title(f'FY2025 Cumulative Revenue with Uncertainty\n({N_SIMULATIONS} Monte Carlo Sims)', 
             fontsize=21, fontweight='bold', pad=20)
...
plt.tight_layout()
monthly_output = OUTPUT_DIR / '10.0.3_MC_monthly_revenue_uncertainty_rebundled.png'
...
print(f"  Average time per simulation: {elapsed/N_SIMULATIONS:.3f}s")
...
print(f"  Std Dev: ${summary_no['total_revenue_std']:,.2f}")
print(f"  Probability of meeting target: {prob_exceed_target:.1f}%")

print(f"\n💰 WITH DISCOUNT Scenario:")
print(f"  Expected revenue: ${summary_with['total_revenue_mean']:,.2f}")
print(f"  {CONFIDENCE_LEVEL*100:.0f}% CI: ${summary_with['total_revenue_ci_lower']:,.2f} - ${summary_with['total_revenue_ci_upper']:,.2f}")
print(f"  Std Dev: ${summary_with['total_revenue_std']:,.2f}")

print(f"\n📁 Output Files:")
print(f"  • 10.0.3_MC_FY2025_comparison_summary_rebundled.csv")
print(f"  • 10.0.3_MC_all_rounds_rebundled.csv")
print(f"  • 10.0.3_MC_detailed_simulations_rebundled.xlsx")
print(f"  • 10.0.3_MC_revenue_distribution_histogram_rebundled.png")
print(f"  • 10.0.3_MC_confidence_intervals_rebundled.png")
print(f"  • 10.0.3_MC_scenario_comparison_boxplot_rebundled.png")
print(f"  • 10.0.3_MC_monthly_revenue_uncertainty_rebundled.png")
...
print(f"  2. Ran {N_SIMULATIONS} Monte Carlo iterations:")
print("     a. Randomly bundled invoices into statements")
print("     b. Assigned deciles based on statement totals")
print("     c. Applied decile base rates (NO seasonal adjustment)")
print("     d. Simulated payment behavior")
print("     e. Calculated revenue for both scenarios")
print("  3. Aggregated results across all simulations")
print("  4. Calculated confidence intervals and distributions")
...
print("\nDifferences from 10.5:")
print("  • NO seasonal adjustments (uses decile base rates directly)")
print("  • Same bundling and payment simulation approach")
print("  • Same uncertainty sources")
print("="*80)