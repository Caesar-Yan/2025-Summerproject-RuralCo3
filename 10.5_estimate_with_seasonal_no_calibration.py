...
# ================================================================
print("\n" + "="*80)
print("📊 REVENUE COMPONENT BREAKDOWN TABLE")
print("="*80)
...
# For this Monte Carlo, we need to extract components from the simulation data
...
table_data = [
    ...
    ['', '', '', ''],
    ['WITH DISCOUNT', f'${total_with_discount:,.0f}', '100%', 'Interest on discounted amounts only'],
    ['', '', '', ''],
    ...
    ['  Total NO DISCOUNT', f'${total_no_discount:,.0f}', '100%', 'Combined interest + retained discounts'],
    ['', '', '', ''],
    ['COMPARISON:', '', '', ''],
    ['Revenue Multiplier', f'{total_no_discount/total_with_discount:.2f}x', f'+{((total_no_discount/total_with_discount-1)*100):.1f}%', 'NO DISCOUNT vs WITH DISCOUNT'],
]
...
for i, row in enumerate(table_data):
    for j, cell in enumerate(row):
        ...
        elif 'COMPARISON' in str(cell):  # Comparison header
            cell_obj.set_facecolor('#F2F2F2')
            cell_obj.set_text_props(weight='bold')
        elif 'Revenue Multiplier' in str(cell):  # Multiplier row
            cell_obj.set_facecolor('#FFF2CC')
            if j == 0:  # Label
                cell_obj.set_text_props(weight='bold')

plt.title('FY2025 Revenue Component Analysis\n(Monte Carlo Simulation Results)', 
          fontsize=16, fontweight='bold', pad=20)

table_output = OUTPUT_DIR / '10.5_revenue_breakdown_table.png'
plt.savefig(table_output, dpi=300, bbox_inches='tight', facecolor='white')
print(f"  ✓ Saved: {table_output.name}")
plt.close()
...
print(f"  • 10.5_revenue_breakdown_table.png")
