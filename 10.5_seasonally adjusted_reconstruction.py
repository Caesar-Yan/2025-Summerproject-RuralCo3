'''
Docstring for 10.5_Calculation_with_seasonal_late_rates

This script makes estimates of revenue generated under discount and no-discount scenarios,
using SEASONALLY ADJUSTED DECILE-SPECIFIC late payment rates. Each decile's base late 
payment rate is multiplied by a seasonal adjustment factor (using November as baseline).
cd levels and payment timing use the real payment profile data.
...
- 10.5_FY2025_seasonal_detailed_simulations.xlsx
- 10.5_cd_level_analysis_seasonal.csv
- 10.5_cumulative_revenue_seasonal.png
- 10.5_monthly_late_rates_comparison.png
- 10.5_decile_seasonal_adjustments.csv
'''
...
# Get November 2025 baseline overall rate
...
if len(november_2025_row) == 0:
    ...
    print("\nAvailable dates:")
    print(seasonal_rates[['invoice_period', 'reconstructed_late_rate_pct']])
    exit(1)

november_baseline_rate = november_2025_row['reconstructed_late_rate_pct'].values[0] / 100

print(f"November 2025 baseline overall late rate: {november_baseline_rate*100:.2f}%")
print(f"This rate will be used to adjust decile-specific rates for other months")

# Calculate seasonal adjustment factors for each month
# CORRECT FORMULA: Factor = Month Rate / November Rate
# Lower spending months have HIGHER rates → factor > 1 (INCREASES decile rates)
# Higher spending months have LOWER rates → factor < 1 (DECREASES decile rates)
...
for _, row in seasonal_rates.iterrows():
    ...
    # CORRECTED: Adjustment factor = This month's rate / November rate
...
for year_month, factor in sorted(seasonal_adjustment_factors.items()):
    month_rate = factor * november_baseline_rate
...
try:
    ...
    print(f"  NOTE: Using decile BASE rates × seasonal adjustment factors")
    print(f"  Profile provides: (1) Base late rates per decile, (2) cd distributions")
    ...
    # Display decile base rates
    print(f"\nDecile Base Late Payment Rates (November baseline):")
    for i in range(n_deciles):
        ...
        if decile_key in decile_profile['deciles']:
            ...
            print(f"  Decile {i}: {base_rate*100:.2f}%")
...
# ================================================================
# Simulation function with seasonally adjusted decile rates
# ================================================================
def simulate_with_seasonal_adjustment(invoices_df, decile_profile_dict, 
                                     seasonal_factors_dict, november_baseline,
                                     discount_scenario, cd_to_days_map):
    """
    Simulate invoice payments using SEASONALLY ADJUSTED decile-specific late rates
    
    KEY APPROACH:
    - Each decile has its own base late payment rate (from profile, November baseline)
    - Apply seasonal adjustment factor to each decile's rate for each month
    - Adjusted Rate = Decile Base Rate × Seasonal Factor
    ...
    - november_baseline: November overall late rate (for reference)
    ...
    decile_base_rate_array = np.zeros(n_invoices, dtype=float)
    seasonal_factor_array = np.zeros(n_invoices, dtype=float)
    adjusted_late_rate_array = np.zeros(n_invoices, dtype=float)
    
    print(f"\nSimulating {n_invoices:,} invoices with SEASONALLY ADJUSTED decile rates...")
    ...
    for idx, row in simulated.iterrows():
        ...
        # ================================================================
        # STEP 1: Get decile's BASE late payment rate (from profile)
        # ================================================================
        ...
        decile_base_rate = decile_data['payment_behavior']['prob_late']
        decile_base_rate_array[idx] = decile_base_rate
        
        # ================================================================
        # STEP 2: Get SEASONAL adjustment factor for this month
        # ================================================================
        if year_month in seasonal_factors_dict:
            seasonal_factor = seasonal_factors_dict[year_month]
            months_used.add(year_month)
        else:
            # Fallback: use 1.0 (no adjustment)
            seasonal_factor = 1.0
        ...
        # ================================================================
        # STEP 3: Calculate ADJUSTED late rate for this invoice
        # ================================================================
        # Adjusted Rate = Decile Base Rate × Seasonal Factor
        adjusted_late_rate = decile_base_rate * seasonal_factor
        ...
        # Determine if payment is late based on ADJUSTED probability
        is_late = np.random.random() < adjusted_late_rate
        is_late_array[idx] = is_late
        
        if is_late:
            # ================================================================
            # STEP 4: If late, get cd distribution from decile profile
            # ================================================================
            ...
            if cd_given_late:
                ...
                # ================================================================
                # STEP 5: Use cd level to determine days overdue
                # ================================================================
    ...
    print(f"  ✓ Applied seasonal adjustments from {len(months_used)} unique months")
    ...
    simulated['decile_base_late_rate'] = decile_base_rate_array
...
# ================================================================
# Run both scenarios with seasonally adjusted decile rates
# ================================================================
print("\n" + "="*70)
print("RUNNING SIMULATIONS WITH SEASONALLY ADJUSTED DECILE RATES")
print("="*70)

# Scenario 1: With discount
print("\nScenario 1: With early payment discount + seasonal adjustment...")
with_discount = simulate_with_seasonal_adjustment(
    fy2025_df, 
    decile_profile, 
    seasonal_adjustment_factors,
    november_baseline_rate,
    discount_scenario='with_discount',
    cd_to_days_map=CD_TO_DAYS
)

# Scenario 2: No discount
print("Scenario 2: No discount + seasonal adjustment...")
no_discount = simulate_with_seasonal_adjustment(
    fy2025_df, 
    decile_profile, 
    seasonal_adjustment_factors,
    november_baseline_rate,
...
# Create summary showing how each decile's rate is adjusted by month
...
print(decile_seasonal_df[decile_seasonal_df['decile'] == 0].to_string(index=False))
...
def print_scenario_summary(df, scenario_name):
    ...
    print(f"  Avg base late rate (deciles): {df['decile_base_late_rate'].mean()*100:.2f}%")
    ...
    # Monthly breakdown
    print(f"\nRevenue by Month (with seasonal adjustments):")
    ...
    # Decile breakdown
    print(f"\nRevenue by Decile (across all months):")
    decile_revenue = df.groupby('decile').agg({
        'interest_charged': 'sum',
        'retained_discounts': 'sum',
        'credit_card_revenue': 'sum',
        'is_late': ['sum', 'count'],
        'decile_base_late_rate': 'first',
        'adjusted_late_rate': 'mean'
    })
    decile_revenue.columns = ['interest', 'retained', 'total_revenue', 'n_late', 'n_invoices', 'base_rate', 'avg_adj_rate']
    decile_revenue['actual_late_pct'] = decile_revenue['n_late'] / decile_revenue['n_invoices'] * 100
    print(decile_revenue.to_string())
    ...
    return {
        ...
        'avg_base_rate': df['decile_base_late_rate'].mean(),
        'avg_adjusted_rate': df['adjusted_late_rate'].mean()
    }

# Print summaries
summary_with = print_scenario_summary(with_discount, "FY2025 - WITH EARLY PAYMENT DISCOUNT (SEASONAL ADJUSTED)")
summary_no = print_scenario_summary(no_discount, "FY2025 - NO DISCOUNT (SEASONAL ADJUSTED + RETAINED DISCOUNTS)")

# ================================================================
# Comparison
# ================================================================
print("\n" + "="*70)
print("DIRECT COMPARISON - FY2025 (SEASONALLY ADJUSTED DECILE RATES)")
print("="*70)
...
print(f"  November 2025 baseline rate: {november_baseline_rate*100:.2f}%")
print(f"  Avg seasonal factor: {summary_with['avg_seasonal_factor']:.3f}")
print(f"  Avg decile base rate: {summary_with['avg_base_rate']*100:.2f}%")
print(f"  Avg adjusted rate: {summary_with['avg_adjusted_rate']*100:.2f}%")

# ================================================================
# Create comparison DataFrame
# ================================================================
comparison_df = pd.DataFrame([summary_with, summary_no])
output_csv = os.path.join(OUTPUT_DIR, '10.5_FY2025_seasonal_comparison_summary.csv')
...
# ================================================================
print("\n" + "="*70)
print("CD LEVEL ANALYSIS (SEASONAL ADJUSTED)")
print("="*70)
...
if cd_analysis:
    ...
    cd_analysis_file = os.path.join(OUTPUT_DIR, '10.5_cd_level_analysis_seasonal.csv')
...
TARGET_REVENUE = 1_043_000  # $1.043M target

# ================================================================
# Visualization 1: Cumulative Revenue Over Time
# ================================================================
...
plt.tight_layout()
viz_path = os.path.join(OUTPUT_DIR, '10.5_cumulative_revenue_seasonal.png')
plt.savefig(viz_path, dpi=300, bbox_inches='tight')
print(f"  ✓ Saved: {viz_path}")
plt.close()

# ================================================================
# Visualization 2: Monthly Late Rates Comparison
# ================================================================
...
monthly_summary.columns = ['invoice_period', 'seasonal_factor', 'avg_adjusted_rate', 'n_late', 'n_invoices']
...
ax1.plot(monthly_summary['invoice_period'], monthly_summary['avg_adjusted_rate'] * 100,
         marker='o', linewidth=2.5, markersize=8, label='Avg Adjusted Late Rate (Expected)',
...
ax1.axhline(y=november_baseline_rate*100, color='red', linestyle=':', linewidth=2,
           label=f'November Baseline ({november_baseline_rate*100:.1f}%)', alpha=0.6)

ax1.set_title('Seasonally Adjusted vs Actual Late Payment Rates\nFY2025', 
...
max_factor_idx = monthly_summary['seasonal_factor'].idxmax()
min_factor_idx = monthly_summary['seasonal_factor'].idxmin()

ax2.annotate(f"Max: {monthly_summary.loc[max_factor_idx, 'seasonal_factor']:.3f}",
            xy=(monthly_summary.loc[max_factor_idx, 'invoice_period'], 
                monthly_summary.loc[max_factor_idx, 'seasonal_factor']),
            xytext=(10, 10), textcoords='offset points',
            fontsize=9, color='purple',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='purple'))

ax2.annotate(f"Min: {monthly_summary.loc[min_factor_idx, 'seasonal_factor']:.3f}",
            xy=(monthly_summary.loc[min_factor_idx, 'invoice_period'], 
                monthly_summary.loc[min_factor_idx, 'seasonal_factor']),
            xytext=(10, -20), textcoords='offset points',
            fontsize=9, color='purple',
            bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='purple'))

plt.tight_layout()
viz_path = os.path.join(OUTPUT_DIR, '10.5_monthly_late_rates_comparison.png')
plt.savefig(viz_path, dpi=300, bbox_inches='tight')
print(f"  ✓ Saved: {viz_path}")
plt.close()

print("\n" + "="*70)
print("ANALYSIS COMPLETE - FY2025 WITH SEASONALLY ADJUSTED DECILE RATES")
print("="*70)
print(f"\nAll results saved to folder: {OUTPUT_DIR}/")
print("Files created:")
print(f"  1. 10.5_FY2025_seasonal_comparison_summary.csv")
print(f"  2. 10.5_FY2025_seasonal_detailed_simulations.xlsx")
print(f"  3. 10.5_cd_level_analysis_seasonal.csv")
print(f"  4. 10.5_cumulative_revenue_seasonal.png")
print(f"  5. 10.5_monthly_late_rates_comparison.png")
print(f"  6. 10.5_decile_seasonal_adjustments.csv")
print("\nSimulation approach:")
print(f"  - November 2025 baseline overall rate: {november_baseline_rate*100:.2f}%")
print(f"  - Each decile has its own base late payment rate")
print(f"  - Seasonal adjustment factors applied to each decile's base rate")
print(f"  - Adjusted Rate = Decile Base Rate × Seasonal Factor")
