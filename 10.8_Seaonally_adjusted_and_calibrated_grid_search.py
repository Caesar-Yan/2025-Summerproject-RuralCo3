'''
Docstring for 10.8_Seasonally_adjusted_and_calibrated_grid_search

This script runs the calibrated baseline simulation MULTIPLE TIMES to generate
distributions and error margins for revenue projections.

Monte Carlo simulation approach:
- Run N simulations with different random seeds
- Each simulation uses the same calibrated baseline and seasonal adjustments
...
Outputs:
- 10.8_monte_carlo_summary_statistics.csv (mean, median, std, CI for both scenarios)
- 10.8_simulation_results_distribution.csv (all N simulation results)
- 10.8_revenue_distribution_histogram.png (distribution visualization)
- 10.8_confidence_intervals.png (CI visualization)
- 10.8_scenario_comparison_boxplot.png (boxplot comparison)
- 10.8_monthly_revenue_uncertainty.png (monthly uncertainty bands)
'''
...
print("\n" + "="*70)
print("MONTE CARLO SIMULATION WITH CALIBRATED BASELINE")
print("="*70)
...
# ================================================================
def simulate_both_scenarios_with_shared_late_status(invoices_df, seed):
    """
    ...
    Returns: (with_discount_results, no_discount_results, monthly_details)
    """
    np.random.seed(seed)
    ...
    # ================================================================
    # STEP 5: Monthly aggregation for uncertainty bands
    # ================================================================
    monthly_with = with_discount.groupby(with_discount['payment_date'].dt.to_period('M')).agg({
    ...
    monthly_no = no_discount.groupby(no_discount['payment_date'].dt.to_period('M')).agg({
        'credit_card_revenue': 'sum'
    }).reset_index()
...
# Summary statistics
summary_output = os.path.join(OUTPUT_DIR, '10.8_monte_carlo_summary_statistics.csv')
...
results_output = os.path.join(OUTPUT_DIR, '10.8_simulation_results_distribution.csv')
...
ax1.set_title('With Discount Revenue Distribution', fontsize=14, fontweight='bold')
...
ax2.set_title('No Discount Revenue Distribution', fontsize=14, fontweight='bold')
...
plt.tight_layout()
hist_output = os.path.join(OUTPUT_DIR, '10.8_revenue_distribution_histogram.png')
...
plt.tight_layout()
ci_output = os.path.join(OUTPUT_DIR, '10.8_confidence_intervals.png')
...
plt.tight_layout()
boxplot_output = os.path.join(OUTPUT_DIR, '10.8_scenario_comparison_boxplot.png')
...
def calculate_monthly_percentiles(monthly_df, percentiles=[0.05, 0.25, 0.50, 0.75, 0.95]):
    ...
    monthly_stats = monthly_df.groupby('payment_date')['cumulative'].quantile(percentiles).unstack()
...
plt.tight_layout()
monthly_output = os.path.join(OUTPUT_DIR, '10.8_monthly_revenue_uncertainty.png')
