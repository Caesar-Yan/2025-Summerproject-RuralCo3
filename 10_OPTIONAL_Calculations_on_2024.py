...
# ================================================================
print("="*70)
print("FINDING DEFAULT RATE TO REACH BREAKEVEN BY END OF 2024")
print("="*70)
...
# Filter to only include data up to end of 2024
combined_df_2024 = combined_df[combined_df['invoice_period'] <= TARGET_DATE].copy()
print(f"\nInvoices through end of 2024: {len(combined_df_2024):,}")
...
# ================================================================
print(f"\nTarget: ${BREAKEVEN_TARGET:,} by {TARGET_DATE.strftime('%Y-%m-%d')}")
...
while iteration < MAX_ITERATIONS:
    ...
    retained, period_summary = calculate_retention_at_date(
        combined_df_2024, mid_rate, TARGET_DATE
    )
...
print(f"Discount retained by end of 2024: ${best_retained:,.2f}")
print(f"Target (breakeven): ${BREAKEVEN_TARGET:,.2f}")
print(f"Difference: ${best_retained - BREAKEVEN_TARGET:+,.2f}")
print(f"\nThis means approximately {int(len(combined_df_2024) * best_rate):,} out of {len(combined_df_2024):,} invoices")
print(f"would need to NOT pay on time for the strategy to break even by end of 2024.")
...
for rate in comparison_rates:
    retained, period_summary = calculate_retention_at_date(
        combined_df_2024, rate, TARGET_DATE
    )
    
    comparison_results.append({
        'default_rate': rate,
        'retained_2024': retained,
        'gap_from_breakeven': retained - BREAKEVEN_TARGET,
        'reaches_breakeven': retained >= BREAKEVEN_TARGET,
        'n_defaults': int(len(combined_df_2024) * rate)
    })
...
for _, row in comparison_df.iterrows():
    ...
    print(f"  Retained: ${row['retained_2024']:,.2f}")
...
ax1.set_title('Cumulative Discount Retention: Finding Breakeven Rate by End of 2024',
...
retention_amounts = [result['retained_2024'] for result in comparison_results]
...
ax2.set_ylabel('Discount Retained by End of 2024 ($)', fontsize=13, fontweight='bold')
ax2.set_title('Default Rate vs. Discount Retained (2024 Only)',
             fontsize=18, fontweight='bold', pad=15)
...
plt.tight_layout()
plt.savefig('breakeven_rate_2024_analysis.png', dpi=300, bbox_inches='tight')
print("✓ Saved visualization to: breakeven_rate_2024_analysis.png")

# ================================================================
# Save results
# ================================================================
comparison_df.to_csv('breakeven_rate_comparison_2024.csv', index=False)
print("✓ Saved comparison results to: breakeven_rate_comparison_2024.csv")

# Save search history
search_df = pd.DataFrame(search_history)
search_df.to_csv('binary_search_history.csv', index=False)
print("✓ Saved search history to: binary_search_history.csv")

# Save detailed period data for optimal rate
best_period_summary.to_csv('optimal_rate_period_detail_2024.csv', index=False)
print("✓ Saved optimal rate period details to: optimal_rate_period_detail_2024.csv")

print("\n" + "="*70)
print("ANALYSIS COMPLETE")
print("="*70)
print(f"\nKEY FINDING: A default rate of {best_rate*100:.3f}% is needed to break even by end of 2024")
