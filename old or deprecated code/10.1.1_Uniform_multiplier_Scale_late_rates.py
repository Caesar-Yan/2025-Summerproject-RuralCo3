'''
Docstring for 10.1.1_Uniform_multiplier_Scale_late_rates

this script does the same job as 10.1_, but instead of assuming a uniform rate for the late incidence,
it scales the late payment profile by multiplying the probibility of a payment being late uniformly across all deciles.
...
outputs:
- 10.1.1_calibration_iterations_MULTIPLIER.csv
- 10.1.1_FY2025_calibrated_MULTIPLIER_detailed.xlsx
- 10.1.1_calibration_summary_MULTIPLIER.csv
- 10.1.1_calibration_results_MULTIPLIER.png
'''
...
calibrated_profile_path = os.path.join(profile_dir, 'decile_payment_profile_CALIBRATED_MULTIPLIER.pkl')
...
iteration_csv = os.path.join(OUTPUT_DIR, '10.1.1_calibration_iterations_MULTIPLIER.csv')
iteration_df.to_csv(iteration_csv, index=False)
print(f"✓ Saved iteration history: {iteration_csv}")

# Save detailed simulations
output_excel = os.path.join(OUTPUT_DIR, '10.1.1_FY2025_calibrated_MULTIPLIER_detailed.xlsx')
...
summary_csv = os.path.join(OUTPUT_DIR, '10.1.1_calibration_summary_MULTIPLIER.csv')
...
plt.tight_layout()
viz_path = os.path.join(OUTPUT_DIR, '10.1.1_calibration_results_MULTIPLIER.png')
